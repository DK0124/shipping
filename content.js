...
(function() {
  ...
  const CONFIG = {
    ...
    PROVIDERS: {
      SEVEN: {
        name: '7-11',
        selector: '.div_frame, table.print_area',
        patterns: {
          order: [
            /訂單編號[：:]\s*([^\s]+)/,
            /交貨便服務代碼[：:]\s*([A-Z0-9-]+)/i,
      ...
      FAMILY: {
        name: '全家',
        selector: '.print-area, .printarea',
        patterns: {
          order: [/訂單編號[：:]\s*([^\s]+)/, /店到店編號[：:]\s*([^\s]+)/],
          store: [/取件門市[：:]\s*([^,\n]+)/]
        }
      },
      DELIVERY: {
  ...
  const state = {
    ...
    pdfShippingData: [],
    shippingDataBatches: [],
    currentPageType: CONFIG.PAGE_TYPES.DETAIL,
  ...
  function injectStyles() {
    ...
    .bv-icon-wrapper {
      width: 40px;
      height: 40px;
      background: linear-gradient(135deg, #518aff 0%, #3b6de0 100%);
      border-radius: 12px;
      display: flex;
      align-items: center;
    ...
    .bv-primary-button:hover {
      transform: translateY(-2px);
      box-shadow: 0 6px 20px rgba(81, 138, 255, 0.3);
    }
  ...
  function detectPageType() {
    ...
    // 檢查是否為物流單頁面
    if (hostname.includes('eservice.7-11.com.tw')) {
        state.currentPageType = CONFIG.PAGE_TYPES.SHIPPING;
        state.currentProvider = 'SEVEN';
    } else if (hostname.includes('ecvip2.family.com.tw')) {
        state.currentPageType = CONFIG.PAGE_TYPES.SHIPPING;
        state.currentProvider = 'FAMILY';
    } else if (hostname.includes('tcat.com.tw') || 
               hostname.includes('t-cat.com.tw') ||
               hostname.includes('tjexpress.tw') ||
               hostname.includes('ecan.com.tw')) {
        state.currentPageType = CONFIG.PAGE_TYPES.SHIPPING;
        state.currentProvider = 'DELIVERY';
        
        // 判斷宅配子類型
        if (hostname.includes('tcat.com.tw') || hostname.includes('t-cat.com.tw')) {
            state.deliverySubType = 'TCAT';
        } else if (hostname.includes('tjexpress.tw')) {
            state.deliverySubType = 'TJ';
        } else if (hostname.includes('ecan.com.tw')) {
            state.deliverySubType = 'ECAN';
        }
    } else {
        // 預設為出貨明細頁面
  ...
  function createControlPanel() {
    ...
    const existingMinButton = document.getElementById('bv-minimized-button');
    if (existingMinButton) {
        existingMinButton.remove();
    }
  ...
  function init() {
    // 檢查是否為目標網址
    if (!window.location.href.includes('sell.9splay.com/SellECOM') &&
        !window.location.hostname.includes('eservice.7-11.com.tw') &&
        !window.location.hostname.includes('ecvip2.family.com.tw') &&
        !window.location.hostname.includes('tcat.com.tw') &&
        !window.location.hostname.includes('t-cat.com.tw') &&
        !window.location.hostname.includes('tjexpress.tw') &&
        !window.location.hostname.includes('ecan.com.tw')) {
        return;
    }
