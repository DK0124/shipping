...
(function() {
    ...
    if (BVWizard.Config.DEBUG) {
        BVWizard.DevTools = {
            // 狀態檢視
            logState() {
                console.group('🔍 BVWizard State');
                console.log('Current:', BVWizard.State.get());
                console.groupEnd();
            },
            ...
            triggerEvent(event, data) {
                ...
                BVWizard.Events.emit(event, data);
            },
            
            // 跳到步驟
            goToStep(stepId) {
                BVWizard.State.set('currentStep', stepId);
                BVWizard.Events.emit('step:change', { step: stepId });
            },
            
            // 填充測試資料
            fillTestData() {
                BVWizard.State.update({
        ...
        // 暴露到全域
        window.BVDev = BVWizard.DevTools;
    }
    
    // 暴露主物件到全域
    window.BVWizard = BVWizard;
    ...
    if (BVWizard.Config.DEBUG) {
        ...
        console.log('🚀 BV SHOP 出貨助手 v' + BVWizard.Config.VERSION + ' 載入完成');
        ...
        console.log('   BVDev.logState()                 - 查看目前狀態');
        console.log('   BVDev.fillTestData()             - 填充測試資料');
    }
